...
if ($id > 0) {
    ...
    if ($result && $result->num_rows > 0) {
        ...
        redirect('index.php', ['status' => 'error', 'msg' => 'Subtask not found!']);
    }
} else {
    redirect('index.php');
}
...
<div class="max-w-2xl mx-auto bg-white p-8 rounded-xl shadow-md border border-gray-100">
    <div class="flex items-center justify-between mb-6">
        <h2 class="text-2xl font-bold text-gray-800">Edit Subtask</h2>
    ...
    <form action="update.php" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <input type="text" name="title"
                   value="<?= htmlspecialchars($old['title'] ?? $subtask['title']) ?>"
        ...
        <div class="flex items-center">
            <input type="checkbox" name="is_completed" id="is_completed" value="1" <?= ($old['is_completed'] ?? $subtask['is_completed']) ? 'checked' : '' ?> class="h-4 w-4 text-blue-600 border-gray-300 rounded">
