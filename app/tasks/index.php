                ...
                <?php if ($result && $result->num_rows > 0): ?>
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4 text-sm text-gray-600"><?= htmlspecialchars($row['project_name'] ?? 'N/A') ?></td>
                            <td class="px-6 py-4 text-sm text-gray-600"><?= htmlspecialchars($row['category_name'] ?? 'N/A') ?></td>
                            <td class="px-6 py-4 text-sm text-gray-600">
                                <span class="px-2 py-1 rounded text-xs font-bold
                                    <?= $row['priority'] == 'high' ? 'bg-red-100 text-red-700' : ($row['priority'] == 'medium' ? 'bg-yellow-100 text-yellow-700' : 'bg-green-100 text-green-700') ?>">
                                    <?= strtoupper($row['priority']) ?>
                                </span>
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-600">
                                <span class="px-2 py-1 rounded text-xs font-bold
                                    <?= $row['status'] == 'done' ? 'bg-blue-100 text-blue-700' : ($row['status'] == 'doing' ? 'bg-orange-100 text-orange-700' : 'bg-gray-100 text-gray-700') ?>">
                                    <?= strtoupper($row['status']) ?>
                                </span>
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-600"><?= $row['due_date'] ?: 'N/A' ?></td>
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="7" class="px-6 py-10 text-center text-gray-500">
                            No tasks found. <a href="create.php" class="text-blue-600 underline">Add your first task</a>.
                        </td>
                    </tr>
                <?php endif; ?>
            </tbody>
